export default async function handler(req, res) {
  ...
  "summary": {
    ...
    const INSTRUCTION = `Génère des supports de révision en FRANÇAIS basés UNIQUEMENT sur le contenu fourni. Réponds UNIQUEMENT avec un JSON valide. Format:\n${JSON_FORMAT}`;
    ...
    if (imageBase64) {
      ...
      // Text mode (includes PDF text extracted client-side)
      const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.GROQ_API_KEY}` },
        body: JSON.stringify({
